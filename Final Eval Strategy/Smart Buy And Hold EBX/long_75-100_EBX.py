...
PRE_DROP_SHORT_SL = 0.4      # <--- NEW: Static SL for the initial short
...
SHORT_TP_ACTIVATION = 6.2     # Profit required to activate trailing
...
def backtest_core(
    ...
    pre_drop_short_sl  # <--- NEW ARGUMENT
):
    """
    ...
    for i in range(n):
        ...
        # 3. Strategy Logic
            ...
            # If we are in the initial short leg waiting for price to drop
                ...
                if position == -1:
                    ...
                    if short_pnl <= -pre_drop_short_sl:
                        ...
                        # We return to neutral state.

                    # 2. Check Target (Open - 0.8)
            ...
            # We just closed the Pre-Drop Short successfully, waiting 5s to go Long
...
def process_day(file_path: str, day_num: int, temp_dir: pathlib.Path, strategy_params) -> str:
    try:
        ...
        signals, positions = backtest_core(
            ...
            ENTRY_DROP_THRESHOLD,
            PRE_DROP_SHORT_SL  # <--- Pass New Parameter
        )
